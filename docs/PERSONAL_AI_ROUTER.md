...
| Google Antigravity | Chat uses a discovered non-interactive print path (`agy --print`) with JSON output when local help proves those flags. Execution is isolated to a scratch working directory under `.opencobalt/scratch/antigravity/`, uses `--sandbox` when discovered, never enables `--dangerously-skip-permissions`, and still flows through `ExecutionEngine`. Authenticated models come from `agy --output-format json models`; disappeared models are not kept available. Local-only requests exclude Antigravity before invocation. Network access and externally managed authentication are required. |
