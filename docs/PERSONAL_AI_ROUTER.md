...
New Chat work starts locally: the plus control focuses the composer without creating an empty record. The new draft starts from Settings defaults, not from another conversation. Changes made in that draft, including Automatic or Manual, provider, model, reasoning, privacy, local-only, fallback, and an optional repository path, stay local until the conversation is stored. Repository attach validates and expands the path first, so the compact header label shows the canonical bound repository before Send without creating an empty conversation. The conversation is stored when the first message is sent or a file is attached. At that moment the visible draft controls become the conversation routing preset and the canonical repository becomes `Conversation.project_path`. An invalid repository path is rejected and the request is not sent without that context. The title starts as "New conversation", is replaced by the first message unless already named, and can be renamed later from the Chat header or conversation list.
...
The shared database means a reset affects more than chat: conversations, personal-AI routes, missions, approvals, receipts, and other ledger-backed state can live in the same file. A conversation workspace path must name an explicit existing directory, is expanded and canonicalized before it is bound, and rejects traversal and dangerously broad roots. The canonical path flows through routing and provider metadata. Coding-capability execution must use that path or fail closed; it must not silently substitute an empty scratch workspace.
...
Desktop Chat layout is a horizontal grid: primary navigation, a bounded conversation column, then the active chat. The optional route inspector remains an overlay. Collapsing the conversation column gives that space to chat. The Chat header and app-navigation affordance remain outside the message scroller. Below 1180px the conversation list is a drawer over chat. Below 1024px primary navigation is also a drawer.

Provider prompts concatenate a system policy and the current user message. The policy includes OpenCobalt product identity, persona controls, execution constraints, the canonical attached repository when present, at most the last ten prior messages truncated to 3000 characters each, and attachment excerpts. Provider and model identity remain provenance rather than first-person assistant identity; a provider-native persona can influence style without changing that boundary. The current user message is not repeated inside history. A first short request therefore has a small OpenCobalt-owned payload; large input-token reports on such requests are mostly provider/runtime baseline context.
...
| Google Antigravity | Chat admission requires a discovered non-interactive print path (`agy --print`), JSON output, and Antigravity's `--sandbox`. General answer-only and research invocations run in an atomically created, unpredictable private mode-0700 OS-temporary workspace outside the repository. An explicit `coding_analysis` request instead runs with the canonical attached repository as its cwd; a missing or invalid bound path blocks before discovery or execution rather than falling back to scratch. Both paths flow through `ExecutionEngine`, never enable `--dangerously-skip-permissions`, and do not claim an OpenCobalt-provided OS sandbox. Authenticated models come from `agy --output-format json models`; disappeared models are not kept available. Local-only requests exclude Antigravity before invocation. Network access and externally managed authentication are required. |
...
The Missions page groups actionable decisions, active work, and outcomes separately. Durable plan-only Auto records remain inspectable under Earlier planning records instead of appearing as current work; no Mission records or architecture are deleted by this presentation filter.

The normal Memory create/edit controls do not offer the legacy `temporary` scope because the persisted model has no TTL or expiration timestamp. Existing records with that scope remain durable and are shown neutrally as saved; editing another field does not silently change their stored scope.
