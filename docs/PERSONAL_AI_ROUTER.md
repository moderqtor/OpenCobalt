...
| Google Antigravity | The `agy` adapter is capability-gated and uses only a discovered non-interactive print mode and discovered model flags. Work still flows through `ExecutionEngine`, and unsafe permission bypass is not enabled. Because this is not a proven inference-only sandbox, answer-only Chat routing excludes it until approval-and-resume exists. Network access and externally managed authentication may be required. |
...
Chat is currently answer-only. Requested tool or skill execution is rejected at the API boundary instead of being silently attempted. The `always_ask` setting also blocks chat model execution because approval-and-resume is not yet implemented; this is an explicit policy denial, not an implicit approval. Other policies allow bounded provider inference while route records retain any human-review requirement before acting on consequential output. Ollama and Mock have inference-only adapter contracts, so harmless discussion of a sensitive topic is distinct from process authority. Codex, Claude, and Antigravity are agent runtimes rather than isolated completion endpoints; Personal AI chat therefore requires explicit approval for those invocations. Because approval-and-resume is not implemented, they currently fail closed in Chat with an inspectable denied route and candidate reasons but no fabricated execution receipt. Their adapters remain usable through explicit receipt-backed execution surfaces whose approval policy permits the requested authority.
