...
| Missions | Existing durable mission records. Chat does not silently create or execute a mission. |
...
|---|---|
| Codex CLI | Execution is enabled only when local help evidence proves the non-interactive `exec` path, a read-only sandbox, and an approval policy of `never`. OpenCobalt sends a read-only planning request through `ExecutionEngine`; it does not enable dangerous bypass, web search, remote control, credential management, MCP management, or repository mutation paths. Codex may still require network access and credentials managed outside OpenCobalt. |
| Google Antigravity | The `agy` executable is capability-gated. Personal-AI execution is available only when discovery proves a safe non-interactive print mode; model or other flags are used only when discovered. Work still flows through `ExecutionEngine`, and unsafe permission bypass is not enabled. Network access and externally managed authentication may be required. |
| Ollama | The local provider is eligible only through a proven loopback endpoint, defaulting to `127.0.0.1:11434`. Model discovery runs through `ExecutionEngine` and produces its own evidence; automatic routes are built from discovered model identifiers, and execution requires an explicit model identifier. A remote or non-loopback endpoint is not treated as local-only. |
| Claude Code | A real execution adapter exists, but no Claude subscription or authenticated session is assumed. Execution requires local help evidence for non-interactive print, text output, and plan permission mode. The adapter does not enable unsafe bypass, browser control, automatic MCP access, deploy, publish, spend, or messaging paths. Network access and externally managed authentication may be required. |
...
| Mock | A deterministic local development provider backed by the noop adapter. It is not a live model and uses simulated chunks. The local Personal AI API explicitly enables it: automatic routing uses it only when no supported real provider is available, and it can be selected manually for development. Its work still crosses `ExecutionEngine` and can produce development receipts. |
...
- **Ollama has no models:** verify the local Ollama runtime is available at the loopback endpoint and has at least one installed model, then refresh provider discovery. OpenCobalt will not invent a default model for personal-AI routing.
