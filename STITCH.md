...
OpenCobalt is a local-first AI orchestration control plane for developers. It routes tasks across Claude Code, Codex CLI, Gemini CLI, Cursor, and Ollama, records decisions in SQLite, and scores completed runs with local telemetry.
...
The CLI should feel comparable in polish to Claude Code, Codex CLI, and Gemini CLI while keeping OpenCobalt's local-first identity.
