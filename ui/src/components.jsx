...
export function RouteInspector({ route, candidates = [], providers = [], personas = [], onClose, onRerun, rerunning, onPromote, promoting, promoted, loading = false, error = null }) {
  ...
  return <div ref={dialogRef} className="route-inspector" role="dialog" aria-modal="true" aria-labelledby="route-inspector-title" tabIndex="-1">
    <div className="inspector-heading"><div><p className="eyebrow">Route receipt</p><h2 id="route-inspector-title">Decision record</h2></div><IconButton label="Close inspector" onClick={onClose}><X size={17} /></IconButton></div>
    <div className="route-sequence" aria-label="Request to route to receipt"><span>request</span><ChevronRight size={13} aria-hidden="true"/><span>route</span><ChevronRight size={13} aria-hidden="true"/><span>receipt</span></div>
    ...
    {!receipt && <div className="notice amber" role="note">No receipt identifier was supplied with this route record.</div>}
...
export function ConversationRail({ conversations, selectedId, onSelect, onCreate, isCreating, disabled = false, mobileOpen = false, onClose }) {
  ...
  return <aside ref={railRef} id="conversation-navigation" className={`conversation-rail ${mobileOpen ? "is-mobile-open" : ""}`} role={mobileOpen ? "dialog" : undefined} aria-modal={mobileOpen ? "true" : undefined} aria-label="Conversations">
    ...
    {!conversations.length && <p className="rail-empty">Create a conversation to keep the route, response, and receipt together.</p>}
  </aside>;
}
